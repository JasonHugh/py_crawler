...
def crawl(url):
    ...
    for child in soup.table.find_all('a'):
        ...
        if ul != None:
            ...
            time = tr[20].td.get_text().replace('\r','').replace('\n','').replace('  ','')
            #写入标题
            num += 1
            content += '<h3><a href='+url+'>'+str(num)+'/'+title+'</a></h3>'+hx+xq+mj+money+time+'<br/>'
            for row in ul.find_all('li'):
                image = row.find('a',{'class':'J_login nail'}).img.get('src')
                image_big = row.find('p').img.get('src')
                #print image_big
                content += '<img data-big="'+image_big+'" data-original="'+image+'" style="margin-right:10px"/>'
        #exit()
        
    return content
    
#写入html文件
write = file('zufang.html','wb+')
content = u'<!doctype html><html><head><meta charset="UTF-8"/><title>租房</title></head><body>'
...
for i in range(1,2):
    ...
    url = 'http://www.19lou.com/thread/category/structure/search/result?m=10001&fid=1637&mf_Q=小河佳苑'
    content += crawl(url)
    print i

content += '<img id="big" style="position:fixed;top:0;right:0;z-index:999;display:none" src=""/><script src="http://apps.bdimg.com/libs/jquery/2.1.4/jquery.min.js"></script><script type="text/javascript" src="http://apps.bdimg.com/libs/jquery-lazyload/1.9.5/jquery.lazyload.js"></script><script>$("img[id!=big]").click(function(){$("#big").attr("src",$(this).attr("data-big"));$("#big").show(500)});$("img[id!=big]").mouseout(function(){$("#big").hide(500)});$("img").lazyload({effect: "fadeIn"});</script></body></html>'
write.write(content.encode('utf-8'))
